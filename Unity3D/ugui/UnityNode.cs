...
namespace Poco
{
    public class UnityNode : AbstractNode
    {
        ...
        public override AbstractNode? GetFieldNameNode()
        {
            ...
            var t = gameObject.transform.TryCast<Transform>();
            if(t != null)
            {
                for (int i = 0; i < t.childCount; i++)
                {
                    var childT = t.GetChild(i);
                    var child = new UnityNode(childT.gameObject);
                    var n = child.GetFieldNameNode();
                    if (n != null)
                        return n;
                }
            }
        ...
        {
            var nameObj = getAttr("name");
            // Console.WriteLine("ContainNodeImpl x0 " + txtObj);
            if (nameObj != null)
            {
                
                string name = (string)nameObj;
                // Console.WriteLine("ContainNodeImpl x1 " + txt);
                if (name == "InputFieldName")
                {
                    // GameSetObjectText(text);
                    // return true;
                    Console.WriteLine("SetFieldNameImpl x1 ");
                    SetText(gameObject, text);
                    return true;
                    // var t2 = gameObject.transform.TryCast<Transform>();
                    // if(t2 != null)
                    // {
                    //     // Console.WriteLine("SetFieldNameImpl x2 ");
                    //     for (int i = 0; i < t2.childCount; i++)
                    //     {
                    //         // Console.WriteLine("SetFieldNameImpl x3 ");
                    //         var childT = t2.GetChild(i);
                    //         var child = new UnityNode(childT.gameObject);
                    //         var nameObj2 = child.getAttr("name");
                    //         if (nameObj2 == null) continue;
                    //         if ((string)nameObj2 == "Text")
                    //         {
                    //             // child.GameSetObjectText(text);
                    //             SetText(childT.gameObject, text);
            ...
            if(t != null)
            {
                for (int i = 0; i < t.childCount; i++)
                {
                    var childT = t.GetChild(i);
                    var child = new UnityNode(childT.gameObject);
                    if (child.SetFieldNameImpl(text))
                        return true;
                }
            }
        ...
        {
            var txtObj = getAttr("text");
            // Console.WriteLine("ContainNodeImpl x0 " + txtObj);
            if (txtObj != null)
            {
                
                string txt = (string)txtObj;
                // Console.WriteLine("ContainNodeImpl x1 " + txt);
        ...
        public override List<AbstractNode> getChildren()
        {
            ...
            var t = gameObject.transform.TryCast<Transform>();
            //Console.WriteLine("getChildren x1 ");
            if(t != null)
            {
                //Console.WriteLine("getChildren x11 " + t.GetType().Name + " " + t.childCount);

                for (int i = 0; i < t.childCount; i++)
                {
                    var child = t.GetChild(i);
                    //Console.WriteLine("GetChild type " + child.GetType().FullName);
                    //Console.WriteLine("getChildren x2");
                    children.Add(new UnityNode(child.gameObject));
                    //Console.WriteLine("getChildren x3");
            ...
            //Console.WriteLine("getChildren x4");
            return children;
        }
        ...
        public void GameSetObjectText(string t)
        {
            TMP_Text tmpText = gameObject.GetComponent<TMP_Text>();
            
            if (tmpText)
            {
                Console.WriteLine("GameSetObjectText 1");
                tmpText.SetText(t);
                return;
            }
            TextMeshProUGUI tmpUIText = gameObject.GetComponent<TextMeshProUGUI>();
            if (tmpUIText)
            {
                Console.WriteLine("GameSetObjectText 2");
                tmpUIText.SetText(t);
                return;
            }
            Text text = gameObject.GetComponent<Text>();
            if (text)
            {
                text.m_Text = t;
                text.text = t;
                Console.WriteLine("GameSetObjectText 3");
            }
            Console.WriteLine("GameSetObjectText 4");
        }
