...
public class PocoManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            try
            {
                ...
                //StartCoroutine((Il2CppSystem.Collections.IEnumerator)AcceptTcpClient());
                //MonoBehaviourExtensions.StartCoroutine(this, AcceptTcpClient());
                //MonoBehaviourExtensions.StartCoroutine(this, AcceptTcpClient());

                Debug.Log(string.Format("mm Tcp server started and listening at {0}", server.Port));
                break;
            }
    ...
    {
        //Il2CppThrea.Dispatch(() =>
        //{
        //    Debug.Log("Start Thread");
        //});
        
        this.server.Start();
        //this.server.AcceptTcpClient2();
    }


    public System.Collections.IEnumerator AcceptTcpClient()
    {
        Debug.Log("start AcceptTcpClient");
        
        while (true)
        {
            this.server.AcceptTcpClient();
            yield return null;
        }
    ...
    {
        Console.WriteLine("run GetScreenSize???");
...
public class RPCParser
{
    ...
    public string HandleMessage(string json)
    {
        ...
        //var data = JsonConvert.DeserializeObject<Dictionary<string, System.Object>>(json, settings);
        //JsonData data = JsonMapper.ToObject(json);
        //JsonDocument doc = JsonDocument.Parse(json);
        //var data = new Dictionary<string, System.Object>();
        Console.WriteLine("??r");

        var data = JsonUtil.ParseObject(json);

        
        if (data.ContainsKey("method") == false)
        {
        ...
        try
        {
            ...
            switch (method)
            {
                ...
                default:
                    List<object> param = (List<object>)data["params"];
                    Console.WriteLine($"{method} {param.Count}");
    ...
    public string formatRequest(string method, object idAction, List<object> param = null)
    {
        ...
        if (param != null)
        {
            data["params"] = JsonUtil.ToJson(param);
        }
